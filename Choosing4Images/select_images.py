...
yolo_4images_path = config[f"{mode}_4images_path"]
...
log_yolo_used_path = f"./log_model_used_frames.txt" 
result_name = os.path.basename(file_path.rstrip("/"))

with open(log_yolo_used_path, "a") as log_file:
    log_file.write(f"{result_name} - {mode}:\n")
    ...
    for j in range (n): 
        ...
        log_file.write(f"{j+1}th fail: {len(used_image_files)}\n")
