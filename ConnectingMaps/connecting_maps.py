...
RC3 = R.from_quat(Q3C).inv().as_matrix()
R32 = (R.from_quat(Q31) * R.from_quat(Q21).inv()).as_matrix() 
R21 = R.from_quat(Q21).as_matrix() 
R1A = R.from_quat(Q1A).as_matrix()
RCA = RC3 * R32 * R21 * R1A

transform_and_save_nextmap_trajectory(nextmap_new_trajectory, RBA, tBA, config["firstmap_new_trajectory_path"], config["final_trajectory_path"])
