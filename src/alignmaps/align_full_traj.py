...
from utils_aligning_maps import load_image_parsed, load_keyframe_trajectory
import yaml
...
def merge_and_transform(prev_traj, curr_traj, output_path):
    ...
    for pose in curr_traj:
        ...
        transformed.append(
            {"timestamp": pose["timestamp"], "translation": t_new.tolist(), "quaternion": q_new.tolist()}
        )

    with open(output_path, "a") as f:
        for pose in transformed:
            line = (
                f"{pose['timestamp']} {pose['translation'][0]} {pose['translation'][1]} {pose['translation'][2]} "
...
for i in range(1, len(sequences)):
    ...
    new_merged_path = os.path.join(
        ...
        "0",
        "txt",
        f"KeyFrameTrajectory{prev_idx}{curr_idx}_new.txt",
    )
    ...
    firstmap_new_traj_path = os.path.join(
        ...
        "0",
        "txt",
        f"KeyFrameTrajectory{prev_idx}_new.txt",
    )
    ...
    nextmap_new_traj_path = os.path.join(
        ...
        "0",
        "txt",
        f"KeyFrameTrajectory{curr_idx}_new.txt",
    )

    image_parsed_txt = os.path.join(
        root_result_dir, "COLMAP", f"{int_curr_idx}th", "four_frame_result", "0", "txt", "images_parsed.txt"
    )
