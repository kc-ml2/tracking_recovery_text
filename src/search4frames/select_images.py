...
from utils_image_selector import load_csv, load_tracking_events, select_images
import yaml
...
csv_path = config[f"filtered_csv_{mode}_path"]
four_images_path = config[f"4images_{mode}_path"]
...
with open(four_images_path, "w") as f:
...
for j in range(n):
    ...
    # (temp) Calculate number of images which used yolo model for time cost evaluation
    used_image_files = set()
    used_image_files.update(select_newmap_images)
    used_image_files.update(select_oldmap_images)
    len_used_image_files = len(used_image_files)
    print(f"\nNumber of images which used {mode}: {len(used_image_files)}")
    ...
    for i, (score, match) in enumerate(best_pair_final):
        ...
        result = compare_best_with_oldmap(
            yolo_data_csv, match, select_newmap_images, img_cache, orb, orb_cache, match_cache
        )
        if result:
    ...
    if valid_results:
        ...
        for idx, (avg_score, img1_file, img2_file, bbox1, bbox2, old_file, old_bbox1, old_bbox2) in enumerate(
            best_pair_data, 1
        ):
            ...
            if idx == 1:
                ...
                kp1, kp2, matches = get_cached_orb_match(
                    img1_file, crop1, bbox1, img2_file, crop2, bbox2, orb, orb_cache, match_cache
                )
            ...
            kp1, kp2, matches1 = get_cached_orb_match(
                img1_file, crop_img1, bbox1, old_file, crop_old1, old_bbox1, orb, orb_cache, match_cache
            )
            ...
            kp1, kp2, matches2 = get_cached_orb_match(
                img2_file, crop_img2, bbox2, old_file, crop_old2, old_bbox2, orb, orb_cache, match_cache
            )
        ...
        with open(four_images_path, "a") as f:
